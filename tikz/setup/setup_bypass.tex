...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        ...
        \draw[rectangle, color = green!60, line width=1mm] (13.5,5) rectangle (14.5,6);
        ...
        \begin{pgfonlayer}{bg}
          ...
          \draw[line width=1pt, color=red] (12,4)--(12,5);
        \end{pgfonlayer}
        \node at (13.5,0) {\ce{HeNe} Laser};
        \node at (15.5,1.5) {Camera};
        \draw (11.75,5) rectangle (12.25,5.5);
        \node at (12,6) {Photodiode};
        \draw[rotate around={45:(14,1.5)}, fill=white] (13.5,1) rectangle (14.5,2);
        \begin{pgfonlayer}{bbg}
          \fill[gray!20] ({14+0.5-sqrt(2)/2},{1.5-0.5+sqrt(2)/2})--(12-.5,3.5-.5)--(12+.5,3.5+.5)--cycle;
        \end{pgfonlayer}
        %pipes
        \begin{pgfonlayer}{bg}    % select the background layer
            ...
            \draw[color = green!60, line width=1mm] (13,3.5) -- (14,3.5) -- (14,5);
    ...
    \end{tikzpicture}
    \caption{Experiment setup with bypass which allows to pump the whole system without changing the opening of the \textsc{Pfeiffer} microvalve. The temperatures within the reservoir and the cell are measure by the thermometers $T_\mathrm{res}$ and $T_\mathrm{cell}$. Furthermore, $P_\mathrm{res}$ and $P_\mathrm{cell}$ are the two installed pressure gauges. The void is experimentally realized by a primary pump. Furthermore, the laser transmission setup and the camera setup are outlined. Only one of the two is set in place at the same time though.}
