...
% erstellt werden.
\newpage
\begin{poliabstract}{Zusammenfassung}
   Um die Prozesse der Kondensation und Evaporation von Hexan in Beengheit zu volumetrisch studieren nutzen wir nanoporöse Aluminiumoxidmembranen. Deren Dicke ist $\SI{30}{\micro\meter}$ und $\SI{60}{\micro\meter}$, die Seitenlänge der gemessenen quadratischen Membrane $\SI{1}{\centi\meter}$. Die Idee ist, Membranen mit monodispersen, zylindrischen Poren, welche entweder an einem oder an beiden Enden geöffnet sind, zu nutzen. Um die Produktion dieser Membranen zu verbessern werden die \textsc{Kelvin}-Gleichungen für große Pordurchmesser ($>\SI{40}{\nano\meter}$) als genügend präzise angenommen. Der Fokus liegt auf dem Produktionsschritt des Ätzens der \textit{Barrierenschicht} (eine Aluminiumoxidschicht, welche die Poren nach der Anodisierung am einen Ende schließt), sowie der Verkleinerung der Porendurchmesser mithilfe von Atomlagenabscheidung. Weiter wird die Abhängigkeit der Qualität der volumetrischen Isothermen von der Porenlänge untersucht.

   Die Wafer zeigen Inhommogenitäten  der Porendurchmesser. Die Poren selbst sind geriffelt und leicht konisch. Diese Dispersionen erscheinen schon vor dem Ätzen der \textit{Barrierenschicht}. Es ist nicht klar, zu welchem Grad sie durch das Anodisieren oder durch das Ätzen des verbleibenden Aluminiums entstehen. Entgegen den Erwartungen greift die für das Ätzen des Aluminiums verwendete Säure Aluminiumoxid an. Weiter zeigt sich, dass durch das Ätzen der \textit{Barrierenschicht} die Isothermen steiler werden, weil die Konizität der Poren beim Aufschwämmen auf Phosphorsäure abnimmt. Dazu kommt, dass die Messungen der dünneren Membranen steilere Isothermen ergeben (das Experiment muss bestätigt werden) - tatsächlich sind es die steilsten Isothermen, die wir bisher gemessen haben.

   Die Atomlagenabscheidung funktioniert für große Porendurchmesser $>\SI{40}{\nano\meter}$. Bei kleineren Porendurchmessern entstehen nur wenig verstandene Defekte. Eine Änderung der verwendeten Parameter könnte dieses Problem aber lösen.
\end{poliabstract}


\begin{poliabstract}{Abstract}
   To study condensation and evaporation of hexane in confinement, we use nanoporous alumina membranes. Their thickness is $\SI{30}{\micro\meter}$ and $\SI{60}{\micro\meter}$, the side lengths of the measured square membranes $\SI{1}{\centi\meter}$. The idea is to have membranes of monodisperse cylindrical pores that are either open on one end or open on both ends by growing perpendicular to the membranes large surfaces. To refine the production of these membranes, the \textsc{Kelvin} equations are assumed to be sufficiently precise for large pore diameters ($>\SI{40}{\nano\meter}$). Focus lies on the production step of the dissolution of the \textit{barrier layer} (a layer of alumina that seals one side of the pores after the anodization) and the pore diameter reduction by atomic layer deposition. Moreover, the dependency of the volumetric isotherms quality on the pore length is regarded.

   The wafers show dispersions of pore diameters and pore quality regarding corrugations and funnellization. These dispersions appear before the \textit{barrier layer} dissolution already. To what degree they are due to the anodization process and respectively the aluminum dissolution of the remaining aluminum remains unclear. In contrast to expectations, the acid used for the aluminum dissolution does attack alumina. Furthermore, I find that the \textit{barrier layer} dissolution steepens the slope of the isotherms, which is due to the reduction of the funnellization during the floating of the membranes on phosphoric acid. In addition to that, the thinner membranes produced much steeper isotherms (the experiment needs confirmation) - in fact the steepest isotherms that we have measured so far.

   Finally, the pore size reduction by atomic layer deposition works for large pore diameters $>\SI{40}{\nano\meter}$. On smaller diameter pores, the process causes not well understood defects. Changing the parameters of the process might solve this problem.
\end{poliabstract}
...
\subfile{chapters/experimental.tex}
\subfile{chapters/experimental_results.tex}
